    ...
    <div class="box-body table-responsive">
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $index+1 }}</td>
                    <td>{{ $deployPlan->{'before_'.$stage} }} <a href="{{ route($route, ['deployment' => $deployPlan->id, 'step' => $stage, 'pos' => 'before']) }}"><i class="piplin piplin-plus"></i></a></td>
                    <td><a href="{{ route($route, ['deployment' => $deployPlan->id, 'step' => $stage]) }}">{{ trans('commands.'.$stage) }}</a> <i class="piplin piplin-info" data-toggle="tooltip" data-placement="right" data-html="true" data-original-title="{!! trans('commands.'.$stage.'_help') !!}"></i></td>
                    <td>{{ $deployPlan->{'after_'.$stage} }} <a href="{{ route($route, ['deployment' => $deployPlan->id, 'step' => $stage, 'pos' => 'after']) }}"><i class="piplin piplin-plus"></i></a></td>
                </tr>
                @endforeach
            </tbody>
        </table>
